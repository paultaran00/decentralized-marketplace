...
async function mint(nftMetadataPath, nftPath){
  ...
  try {
    let receipt = await TokenInstance.methods.createArtwork(nftMetadataPath, royalty).send({from: user.attributes.ethAddress});
    $('#saveToIPFSBtn').prop('disabled', true);
...
async function uploadToDB(tokenId, nftMetadataPath, nftPath){
  $('#saveToIPFSBtn').html(`Finishing upload <div class="spinner-border spinner-border-sm text-light" role="status">
                                                            <span class="sr-only">Loading...</span>
                                                          </div>`);
  try {
    ...
    artwork.set('tokenAddress', TokenAddress.toLowerCase());
...
function setArtForSale(tokenId){
  $('#setOffer').click(async() =>{
    ...
    try{
      await MarketplaceInstance.methods.setOffer(amountInWei, tokenId, TokenAddress).send({from: user.attributes.ethAddress});
