...
async function ifAddressNotInDatabase(address){
  ...
  if(!isAddressIn){
    $('.profileGenerated').html(`<p id="notYetConnectedText">This address has not yet connected their wallet</p>`);
  }
};
...
function twitterShareProfile(){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20artist%20on%20descentralizedassets!&hashtags=descentralizedassets%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=descentralizedassets&url=${page}`;
  $('#twitterShare').click(()=>{
...
function facebookShareProfile(username){
  ...
  let height = screen.height / 3;
  let page = window.location.href;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${page}%3F&quote=Check%20out%20${username}'s%20NFTs%20on%20descentralizedassets`;
  $('#facebookShare').click(()=>{
...
function emailShareProfile(username){
  let page = window.location.href;
  $('#emailShare').click(()=>{
    window.location.href = `mailto:user@example.com?subject=${username}'s%20profile%20on%20descentralizedassets&body=Check%20out%20the%20artwork%20of%20${username}%20on%20descentralizedassets,%20${page}`;
  });
};
...
function shareOptions(tokenAddress, id){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20NFT%20on%20descentralizedassets!&hashtags=descentralizedassets%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=descentralizedassets&url=${tokenPage}`;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${tokenPage}%3F&quote=Check%20out%20this%20NFT%20on%20descentralizedassets`;
  ...
  $('#emailBtnInModal' + tokenAddress + id).click(()=>{
    window.location.href = `mailto:user@example.com?subject=Check%20out%20this%20NFT%20on%20descentralizedassets&body=Never%20seen%20anything%20quite%20like%20this,%20${tokenPage}`;
  });
};
...
async function checkForApprovalBeforePuttingOnSale(tokenAddress, id){
  try{
    let approved = await TokenInstance.methods.isApprovedForAll(user.attributes.ethAddress, MarketplaceAddress).call();
...
function setApprovalIfNeeded(tokenAddress, id){
  $('#setApprovalBtn' + tokenAddress + id).click(async() =>{
    ...
    await TokenInstance.methods.setApprovalForAll(MarketplaceAddress, true).send({from: user.attributes.ethAddress}, (err, txHash) => {
      if(err){
        alert(err.message);
...
function putOnSale(tokenAddress, id, royalty, creator){
  $('#putOnSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await MarketplaceInstance.methods.setOffer(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function removeFromSale(tokenAddress, id, royalty, creator){
  ...
  $('#removeFromSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await MarketplaceInstance.methods.removeOffer(id, tokenAddress).send({from: user.attributes.ethAddress});
...
function changePriceFrontEnd(tokenAddress, id, royalty, creator){
  $('#changePriceBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await MarketplaceInstance.methods.changePrice(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function transferToken(tokenAddress, id){
  $('#transferTokenBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await TokenInstance.methods.safeTransferFrom(user.attributes.ethAddress, toAddress, id).send({from: user.attributes.ethAddress});
...
function cardDiv(tokenAddress, id, owner){
  let nftCard = `<div id="nftCard`+tokenAddress+id+`" class="grid-helper col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-2">
