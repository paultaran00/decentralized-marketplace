...
async function buy(tokenAddress, id, price, royalty, creator){
  $('#buy' + tokenAddress + id).click(async function(){
    if(user){
      ...
      await MarketplaceInstance.methods.buyArt(id, tokenAddress).send({from: ethereum.selectedAddress, value: price}, (err, txHash) => {
        if(err){
          alert(err.message);
...
function copyButton(){
  let clipboard = new ClipboardJS('#copyUnlockableContentBtn',{container: document.getElementById('unlockableContentModal')});
