...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active && followingArray.includes(item.creator)).length;
  if(activeCount < 1){
    $('.following-wrapper').html(`<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork for sale at the moment.<div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active && followingArray.includes(item.creator)).length;
  if(inactiveCount < 1){
    $('.following-wrapper').html(`<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork not for sale at the moment.<div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.following-wrapper').html(`<div class="no-art-for-sale shadow-sm">The artists you follow do not have artwork created at the moment.<div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
