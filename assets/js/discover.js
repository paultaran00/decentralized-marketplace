...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active).length;
  if(activeCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork for sale, but you can change that <a class="gradient-text" href="erc-721.html"> here!<a><div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  if(inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork not for sale, but you can change that <a class="gradient-text" href="erc-721.html"> here!<a><div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork, but you can change that <a class="gradient-text" href="erc-721.html"> here!<a><div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
...
function cardDiv(tokenAddress, id, owner){
  let nftCard = `<div id="nftCard`+tokenAddress+id+`" class="grid-helper col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
